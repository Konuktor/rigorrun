...
| File | What it is |
|---|---|
| `trace.json` | A sanitised recording of one human doing a refund in Northstar Support. 7 semantic steps, no page HTML, no credentials. |
...
| `benchmark.json` | 22 executable cases across nine categories, generated from the contract. `checks` is the private verifier configuration and is never shown to an agent. |
...
pnpm rigorrun gate examples/refund-workflow/benchmark.json --agent reference
```
