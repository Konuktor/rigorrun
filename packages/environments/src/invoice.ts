...
const schema: EnvironmentSchema = {
  entities: [
    entity(
      'Vendor',
      'vendorId',
      [id('vendorId'), plain('name', 'Name'), status('standing', ['active', 'suspended'], 'Standing')],
      { label: 'vendor', mutable: false },
    ),
    entity(
      'PurchaseOrder',
      'poId',
      [id('poId'), ref('vendorId'), money('amount', 'Amount'), status('poStatus', ['open', 'closed'], 'Status')],
    ...
    entity(
      ...
      [
        ...
        money('amount', 'Amount'),
        status('invoiceStatus', ['received', 'approved', 'rejected', 'paid'], 'Status'),
        actor('submittedBy', { label: 'Submitted by' }),
        actor('approvedBy', { label: 'Approved by' }),
...
export const invoiceEnvironment = defineEnvironment({
  ...
  presentation: {
    ...
    navLabels: { Invoice: 'Invoices', PurchaseOrder: 'Purchase orders', Vendor: 'Vendors' },
    actionLabels: {
      requestManagerApproval: 'Request approval',
      approveInvoice: 'Approve for payment',
      writeLedgerEntry: 'Post to ledger',
    },
    focusEntity: 'Invoice',
    statusTones: {
      received: 'progress',
    ...
    entities: [
      {
        ...
        columns: [
          { field: 'invoiceNumber', label: 'Number', emphasis: true },
          { field: 'vendor__name', label: 'Vendor', width: 'wide' },
          { field: 'amount', label: 'Amount', align: 'end' },
          { field: 'invoiceStatus', label: 'Status', width: 'narrow' },
          { field: 'approvedBy', label: 'Approved by' },
        ],
        sections: [
          { title: 'Invoice', fields: ['invoiceNumber', 'amount', 'invoiceStatus'] },
          { title: 'Matching', fields: ['vendor__name', 'purchaseOrder__amount', 'purchaseOrder__poStatus'] },
          { title: 'Authorisation', fields: ['submittedBy', 'approvedBy', 'approval__approvalStatus'] },
          { title: 'Supplier memo', fields: ['memo'], kind: 'prose' },
        ],
        actions: ['requestManagerApproval', 'approveInvoice', 'writeLedgerEntry'],
      },
      {
        ...
        view: 'table',
        columns: [
          { field: 'poId', label: 'PO', emphasis: true },
          { field: 'vendor__name', label: 'Vendor', width: 'wide' },
          { field: 'amount', label: 'Committed', align: 'end' },
          { field: 'poStatus', label: 'Status', width: 'narrow' },
        ],
        sections: [{ title: 'Purchase order', fields: ['poId', 'amount', 'poStatus', 'vendor__name'] }],
      },
      {
        entity: 'Vendor',
        plural: 'Vendors',
        view: 'list',
        columns: [
          { field: 'name', label: 'Vendor', emphasis: true },
          { field: 'standing', label: 'Standing', width: 'narrow' },
        ],
        sections: [{ title: 'Vendor', fields: ['vendorId', 'name', 'standing'] }],
      },
    ],
  },
  caseConfig: [
