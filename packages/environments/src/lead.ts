  ...
  entities: [
    ...
    entity(
      ...
      [
        ...
        status('reviewStatus', ['pending', 'cleared', 'blocked'], 'Status'),
      ],
    ...
    entity(
      ...
      [
        ...
        plain('email', 'Email'),
        status('leadStatus', ['new', 'working', 'qualified', 'disqualified'], 'Stage'),
        status('tier', ['standard', 'enterprise'], 'Tier'),
        actor('ownerId', { label: 'Owner' }),
        actor('qualifiedBy', { label: 'Qualified by' }),
...
export const leadEnvironment = defineEnvironment({
  ...
  presentation: {
    ...
    navLabels: { Lead: 'Pipeline', Company: 'Accounts' },
    actionLabels: {
      requestComplianceReview: 'Request review',
      qualifyLead: 'Qualify lead',
      writeActivity: 'Log activity',
    },
    focusEntity: 'Lead',
    statusTones: {
      new: 'neutral',
    ...
    entities: [
      {
        entity: 'Lead',
        plural: 'Pipeline',
        // A sales pipeline is a board. Nothing in the renderer knows that;
        // this line is why it looks like one.
        view: 'board',
        groupBy: 'leadStatus',
        subtitleField: 'email',
        columns: [
          { field: 'company__companyName', label: 'Company', emphasis: true },
          { field: 'tier', label: 'Tier', width: 'narrow' },
          { field: 'company__employees', label: 'Staff', align: 'end' },
          { field: 'ownerId', label: 'Owner' },
        ],
        sections: [
          { title: 'Lead', fields: ['email', 'leadStatus', 'tier', 'ownerId', 'qualifiedBy'] },
          {
            title: 'Account',
            fields: ['company__companyName', 'company__domain', 'company__employees', 'company__restrictedRegion'],
          },
          { title: 'Compliance', fields: ['review__reviewStatus', 'review__decidedBy'] },
          { title: 'Inbound note', fields: ['inboundNote'], kind: 'prose' },
        ],
        actions: ['requestComplianceReview', 'qualifyLead', 'writeActivity'],
      },
      {
        entity: 'Company',
        plural: 'Accounts',
        view: 'table',
        columns: [
          { field: 'companyName', label: 'Company', emphasis: true },
          { field: 'domain', label: 'Domain', width: 'wide' },
          { field: 'employees', label: 'Staff', align: 'end' },
          { field: 'restrictedRegion', label: 'Restricted', width: 'narrow' },
        ],
        sections: [
          { title: 'Account', fields: ['companyName', 'domain', 'employees', 'restrictedRegion'] },
        ],
      },
    ],
  },
