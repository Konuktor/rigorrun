  ...
  entities: [
    entity(
      ...
      [
        ...
        status('employment', ['active', 'on_leave', 'terminated']),
    ...
    entity(
      'Tool',
      'toolId',
      [id('toolId'), plain('toolName', 'tool'), flag('privileged', 'administrator tool')],
    ...
    entity(
      ...
      [
        id('approvalId'),
        actor('decidedBy', { label: 'security officer' }),
        status('approvalStatus', ['pending', 'cleared', 'refused']),
      ],
    ...
    entity(
      ...
      [
        ...
        days('grantedOnDay', 'granted on day'),
        days('expiresOnDay', 'expires on day'),
        actor('grantedBy'),
        status('grantStatus', ['active', 'revoked']),
        text('justification', { untrusted: true }),
      ],
      { label: 'access grant' },
    ),
...
export const accessEnvironment = defineEnvironment({
  ...
  presentation: {
    ...
    accent: '#b45309',
    mark: 'LI',
    navEntities: ['AccessGrant', 'Employee', 'Tool'],
