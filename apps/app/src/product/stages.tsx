...
/**
 * How the agent reaches the system, asked the way somebody already knows the
 * answer.
 *
 * This was a `<select>` of three lines reading "MCP — a server that publishes
 * tools". That asks a first-time user to place their own system in a taxonomy
 * they have not learned yet, at the first screen, before anything has paid off.
 * The question they can actually answer is what their agent *does*: does it
 * call tools, call an API, or click around a page. So that is the question,
 * with the protocol name kept as the label of the answer rather than the
 * question.
 *
 * Each option states the verification consequence, because it is the one
 * decision on this screen that cannot be changed later without redoing the
 * demonstration.
 */
const CONNECTORS = [
  {
    ...
    detail: 'A server you already run for Claude, Cursor or anything else.',
    strength: 'Can read state back',
    strong: true,
  },
  {
    value: 'openapi',
    ...
    detail: 'An OpenAPI document, and the address requests actually go to.',
    strength: 'Can read state back',
    strong: true,
  },
  {
    ...
    detail: 'A page a person would use. RigorRun drives a browser and watches.',
    strength: 'Watches only',
    strong: false,
  },
] as const;

function ConnectorChoice({
  kind,
  onChange,
}: {
  kind: 'mcp' | 'openapi' | 'browser';
  onChange: (next: 'mcp' | 'openapi' | 'browser') => void;
}) {
  return (
    <fieldset className="border-0 p-0">
      <legend className="mb-2.5 text-body font-medium text-fg">What does your agent do?</legend>
      <div className="grid gap-2 sm:grid-cols-3">
        {CONNECTORS.map((option) => {
          ...
          return (
            <label
              key={option.value}
              className={`flex cursor-pointer flex-col gap-1 rounded-panel border p-4 transition-colors ${
                selected
            ...
            >
              <input
                ...
                data-testid={`connector-kind-${option.value}`}
                className="sr-only"
              />
              <span className="text-body font-medium text-fg">{option.title}</span>
              <span className="text-meta text-secondary">{option.lede}</span>
              <span className="mt-1 text-meta text-muted">{option.detail}</span>
              <span
                className={`mt-2.5 border-t border-line-soft pt-2 text-micro font-semibold uppercase ${
                  option.strong ? 'text-pass' : 'text-warn'
                }`}
              >
                {option.strength}
              </span>
      ...
      {/*
        The old select carried this as a hidden implication. A person choosing
        the browser lane is choosing a weaker verdict, and finding that out at
        the verdict screen is finding it out too late.
      */}
      <p className="mt-3 max-w-2xl text-meta text-muted">
        Reading the system back afterwards is what makes a verdict worth having. A browser can
        watch what your agent does, and you can attach an MCP server or an API for the same system
        later to have the result checked against records.
      </p>
    </fieldset>
  );
}
  ...
  return (
    <Panel>
      <div className="flex max-w-2xl flex-col gap-5">
        ...
        <ConnectorChoice kind={kind} onChange={setKind} />
