...
test.describe('golden path', () => {
  test('demo → contract → benchmark → run → verdict', async ({ page }) => {
    const watchers = watchPage(page);

    /*
     * This used to open on the landing page and assert three of its sentences.
     * The landing page is a separate static build now and lives at
     * rigorrun.xyz; `e2e/site.spec.ts` asserts its claims against the real
     * deployment. What is left here is the demo journey, which is what this
     * file is for.
     */
